...
func main() {
	for {
		var key []byte;var keysize int;var fis string;var keyname string;var pro string;var choice int;var wg sync.WaitGroup
		fmt.Scanf("Choose :\n1. Create key\n2. Encrypt\n3. Decrypt\n4. Exit %d",&choice)
		switch choice {
		case 4:return
		case 3:fmt.Scanf("Keyfile : %s",&keyname);key = ee(keyname);fmt.Println("Files : ");fileList := strings.Fields(fis);fmt.Println(fmt.Sprintf("Using key %s to decrypt files: %s", keyname, fileList));fmt.Println("Is that correct? (y/n)");fmt.Scanln(&pro);if strings.ToLower(pro) != "y" {fmt.Println("Aborting...");return} else {wg.Add(1);ccs(key, fileList, &wg);wg.Wait();fmt.Println("Successfully decrypted files!")}
		case 2:fmt.Scanf("Keyfile : %s",&keyname);key = ee(keyname);fmt.Println("Files : ");fmt.Scanln(&fis);fileList := strings.Fields(fis);fmt.Println(fmt.Sprintf("Using key %s to encrypt files: %s", keyname, fileList));fmt.Println("Is that correct? (y/n)");fmt.Scanln(&pro);if strings.ToLower(pro) != "y" {fmt.Println("Aborting...");return} else {wg.Add(1);bbs(key, fileList, &wg);wg.Wait();fmt.Println("Successfully encrypted files!")}
		case 1:
